...
contains
  ...
  subroutine history_init()
    ...
    call io_add_dim('lat',  size=mesh%num_half_lat, add_var=.true.)
    call io_add_dim('ilon', size=mesh%num_half_lon, add_var=.true.)
    call io_add_dim('ilat', size=mesh%num_full_lat, add_var=.true.)
    ...
    call io_add_dim('lat',  'debug', size=mesh%num_half_lat, add_var=.true.)
    call io_add_dim('ilon', 'debug', size=mesh%num_half_lon, add_var=.true.)
    call io_add_dim('ilat', 'debug', size=mesh%num_full_lat, add_var=.true.)
  ...
  subroutine history_write_state(state, static, diag)
    ...
    call io_output('lat',   mesh%half_lat_deg(:))
    call io_output('ilon',  mesh%half_lon_deg(:))
    call io_output('ilat',  mesh%full_lat_deg(:))
    call io_output('u',     u(1:mesh%num_half_lon,1:mesh%num_full_lat))
    call io_output('v',     v(1:mesh%num_full_lon,1:mesh%num_half_lat))
    call io_output('gh',    gh(1:mesh%num_full_lon,1:mesh%num_half_lat))
    call io_output('ghs',   static%ghs(1:mesh%num_full_lon,1:mesh%num_half_lat))
    call io_output('vor',   diag%vor(1:mesh%num_half_lon,1:mesh%num_full_lat))
    call io_output('div',   diag%div(1:mesh%num_full_lon,1:mesh%num_half_lat))
