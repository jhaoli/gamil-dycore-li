...
contains
  ...
  subroutine history_init()
    ...
    call io_add_var('tes',  long_name='total potential enstrophy', units='s-2 m-1', dim_names=['time'])
  ...
  subroutine history_write_state(state, static, diag)
    ...
    call io_output('lon', mesh%full_lon_deg(1:mesh%num_full_lon))
    call io_output('lat', mesh%full_lat_deg(1:mesh%num_full_lat))
    call io_output('u',   state%u(1:mesh%num_full_lon,1:mesh%num_full_lat))
    call io_output('v',   state%v(1:mesh%num_full_lon,1:mesh%num_full_lat))
    call io_output('gd',  state%gd(1:mesh%num_full_lon,1:mesh%num_full_lat))
    call io_output('ghs', static%ghs(1:mesh%num_full_lon,1:mesh%num_full_lat))
    call io_output('vor', diag%vor(1:mesh%num_full_lon,1:mesh%num_full_lat))
