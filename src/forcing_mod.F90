...
contains
  subroutine force_run(state, tend)
    ...
    real, parameter    :: tau_u = 100*86400.0, &
                          tau_h = 100*86400.0
    real :: ueqm, heqm, h0
    integer :: i, j
    ...
    case ('held_suarez')
      ...
      do j = parallel%full_lat_start_idx_no_pole, parallel%full_lat_end_idx_no_pole
        ...
        do i = parallel%half_lon_start_idx, parallel%half_lon_end_idx
          tend%force%u(i,j) = state%u(i,j) / (state%iap%gd(i,j) + state%iap%gd(i+1,j)) * (ueqm*g - 0.5*(state%gd(i,j) + state%gd(i+1,j))) / tau_h + &
                              0.5 * (state%iap%gd(i,j) + state%iap%gd(i+1,j)) * (ueqm - state%u(i,j)) / tau_u
        end do 
      end do 
      call parallel_fill_halo(tend%force%u, all_halo=.true.)

      do j = parallel%half_lat_start_idx, parallel%half_lat_end_idx
        heqm = h0 + dh * (1 - mesh%half_sin_lat(j)**2)
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%force%v(i,j) = state%v(i,j) / (state%iap%gd(i,j) + state%iap%gd(i,j+1)) * (heqm * g - state%gd(i,j)) / tau_h
        end do 
      end do 
