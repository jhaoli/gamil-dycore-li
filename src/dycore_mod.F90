      ...
      do j = parallel%half_lat_start_idx, parallel%half_lat_end_idx
        ...
        if (filter_half_zonal_tend(j)) then
          ...
          if (abs(s1) > 1.0e-16) then
            call filter_array_at_half_lat(j, tend%du(:,j))
    ...
    case (fast_pass)
      ...
      do j = parallel%half_lat_start_idx, parallel%half_lat_end_idx
        ...
        if (filter_half_zonal_tend(j)) then
          ...
          if (abs(s1) > 1.0e-16) then
            call filter_array_at_half_lat(j, tend%mass_div_lon(:,j))
          ...
          if (abs(s1) > 1.0e-16) then
            call filter_array_at_half_lat(j, tend%mass_div_lat(:,j))
            s2 = sum(tend%mass_div_lat(i1:i2,j) * (state%gd(i1:i2,j) + static%ghs(i1:i2,j))) * mesh%full_cos_lat(j)
            if (filter_full_zonal_tend(j) .and. filter_full_zonal_tend(j+1)) then
            ...
            else if (filter_full_zonal_tend(j+1)) then
              filter_residue_mass_div_lat(j+1) = filter_residue_mass_div_lat(j+1) + s1 - s2
            else if (filter_full_zonal_tend(j)) then
      ...
      do j = parallel%half_lat_start_idx, parallel%half_lat_end_idx
        ...
        if (filter_half_zonal_tend(j)) then
          ...
          if (abs(s1 + filter_residue_mass_div_lon(j)) > 1.0e-16) then
            call filter_array_at_half_lat(j, tend%u_pgf(:,j))
      ...
      do j = parallel%full_lat_start_idx, parallel%full_lat_end_idx
        ...
        if (filter_full_zonal_tend(j)) then
          ...
          if (abs(s1 + filter_residue_mass_div_lat(j)) > 1.0e-16) then
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(c1, c2, i, k, reduced_tend)
    do j = parallel%half_lat_start_idx, parallel%half_lat_end_idx
      ...
      if (half_reduce_factor(j) == 1) then
        do i = parallel%half_lon_start_idx, parallel%half_lon_end_idx
          tend%fv(i,j) = 0.25 * coef%half_f(j) * &
        ...
        do k = 1, half_reduce_factor(j)
          do i = reduced_start_idx_at_half_lat(j), reduced_end_idx_at_half_lat(j)
            reduced_tend(i) = 0.25 * coef%half_f(j) * &
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(i, k, reduced_tend)
    do j = parallel%full_lat_start_idx_no_pole, parallel%full_lat_end_idx_no_pole
      ...
      if (half_reduce_factor(j) == 1) then
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%fu(i,j) = 0.25 * coef%half_f(j) * (state%iap%u(i,j) + state%iap%u(i-1,j))
        end do
      else
        do k = 1, half_reduce_factor(j)
          do i = reduced_start_idx_at_half_lat(j), reduced_end_idx_at_half_lat(j)
            reduced_tend(i) = 0.25 * coef%half_f(j) * (half_reduced_state(j)%iap%u(i,k,2) + half_reduced_state(j)%iap%u(i-1,k,2))
          end do
      ...
      if (half_reduce_factor(j-1) == 1) then
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%fu(i,j) = tend%fu(i,j) + 0.25 * coef%half_f(j-1) * (state%iap%u(i,j-1) + state%iap%u(i-1,j-1))
        end do
      else
        ...
        do k = 1, half_reduce_factor(j-1)
          do i = reduced_start_idx_at_half_lat(j-1), reduced_end_idx_at_half_lat(j-1)
            reduced_tend(i) = 0.25 * coef%half_f(j-1) * (half_reduced_state(j-1)%iap%u(i,k,2) + half_reduced_state(j-1)%iap%u(i-1,k,2))
          end do
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(c1, c2, i, k, reduced_tend)
    do j = parallel%half_lat_start_idx, parallel%half_lat_end_idx
      ...
      if (half_reduce_factor(j) == 1) then
        do i = parallel%half_lon_start_idx, parallel%half_lon_end_idx
          tend%cv(i,j) = 0.25 * coef%half_c(j) * state%u(i,j) * &
            (c1 * (state%iap%v(i,j  ) + state%iap%v(i+1,j  )) + &
             c2 * (state%iap%v(i,j+1) + state%iap%v(i+1,j+1)))
        end do
      else
        ...
        do k = 1, half_reduce_factor(j)
          do i = reduced_start_idx_at_half_lat(j), reduced_end_idx_at_half_lat(j)
            reduced_tend(i) = 0.25 * coef%half_c(j) * half_reduced_state(j)%u(i,k,2) * &
              (c1 * (half_reduced_state(j)%iap%v(i,k,2) + half_reduced_state(j)%iap%v(i+1,k,2)) + &
               c2 * (half_reduced_state(j)%iap%v(i,k,3) + half_reduced_state(j)%iap%v(i+1,k,3)))
          end do
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(i, k, reduced_tend)
    do j = parallel%full_lat_start_idx_no_pole, parallel%full_lat_end_idx_no_pole
      ...
      if (half_reduce_factor(j) == 1) then
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%cu(i,j) = 0.25 * coef%half_c(j) * (state%u(i,  j) * state%iap%u(i,  j) + &
                                                state%u(i-1,j) * state%iap%u(i-1,j))
        end do
      else
        do k = 1, half_reduce_factor(j)
          do i = reduced_start_idx_at_half_lat(j), reduced_end_idx_at_half_lat(j)
            reduced_tend(i) = 0.25 * coef%half_c(j) * &
      ...
      if (half_reduce_factor(j-1) == 1) then
        do i = parallel%full_lon_start_idx, parallel%full_lon_end_idx
          tend%cu(i,j) = tend%cu(i,j) + 0.25 * coef%half_c(j-1) * (state%u(i,  j-1) * state%iap%u(i,  j-1) + &
        ...
        do k = 1, half_reduce_factor(j-1)
          do i = reduced_start_idx_at_half_lat(j-1), reduced_end_idx_at_half_lat(j-1)
            reduced_tend(i) = 0.25 * coef%half_c(j-1) * &
